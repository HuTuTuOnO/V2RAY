#!/bin/bash
# v2ray一键安装脚本
# Author: HuTuTu<https://www.52lz.cn/>
...
module_hotfixes=true' > /etc/yum.repos.d/nginx.repo
        ...
        $CMD_INSTALL nginx
        if [[ "$?" != "0" ]]; then
            colorEcho $RED " Nginx安装失败，请到 https://www.52lz.cn/ 反馈"
            exit 1
        fi
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，请复制上面的红色文字到 https://www.52lz.cn/ 反馈"
            exit 1
        }
        CERT_FILE="/etc/v2ray/${DOMAIN}.pem"
        KEY_FILE="/etc/v2ray/${DOMAIN}.key"
        ...
        [[ -f $CERT_FILE && -f $KEY_FILE ]] || {
            colorEcho $RED " 获取证书失败，请到 https://www.52lz.cn/ 反馈"
            exit 1
        }
    else
...
Documentation=https://www.52lz.cn/
...
menu() {
    ...
    echo -e "# ${GREEN}网址${PLAIN}: https://www.52lz.cn                 #"
    echo "#############################################################"
